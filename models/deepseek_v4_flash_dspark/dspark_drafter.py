...
# -----------------------------------------------------------------------------------------------------------
# ci: devices=2
"""Compose the existing DSpark leaf operators into a three-layer draft backbone.

The caller owns request metadata. This file only connects the existing projection,
context-KV, embedding, HC, attention, MoE, and HC-head implementations; it does not
duplicate their math or implement serving-side verification and rejection.
"""
...
def dspark_drafter(
    ...
    head_hidden[0:QUERY_TOKENS, 0:D] = head_padded[0:QUERY_TOKENS, 0:D]
    return head_hidden
...
def l3_dspark_drafter(
    ...
    for rank in pl.range(pld.world_size()):
        ...
        dspark_drafter(
            ...
            hc_head_fn[rank], hc_head_scale[rank], hc_head_base[rank], head_hidden[rank],
            recv_meta, recv_x, recv_aux, recv_route,
            arrived, data_arrived, routed_y_buf, combine_arrived,
            rank,
            device=rank,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Validate the composed DSpark draft backbone.")
