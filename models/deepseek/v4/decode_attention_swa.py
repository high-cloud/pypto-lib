...
def attention_swa(
    ...
    cache_insert_tids = pl.array.create(1, pl.TASK_ID)
    kv_cache_flat = pl.reshape(kv_cache, [B * ORI_MAX_BLOCKS * BLOCK_SIZE, HEAD_DIM])
    with pl.spmd(T, name_hint="swa_cache_insert") as cache_insert_tid:
        write_t = pl.tile.get_block_idx()
        write_row_i64 = pl.read(swa_slot_mapping, [write_t])
        if write_row_i64 >= 0:
            write_row = pl.cast(write_row_i64, pl.INDEX)
            kv_cache_flat[write_row : write_row + 1, 0 : HEAD_DIM] = kv[write_t : write_t + 1, 0 : HEAD_DIM]
    cache_insert_tids[0] = cache_insert_tid

    attn_out = pl.create_tensor([T, D], dtype=pl.BF16)
    sparse_attn_swa(
        q, kv_cache_flat, swa_indices, swa_lens, cache_insert_tids,
