...
def history_window_swa_indices_and_lens(
    ...
    *,
    ...
    returned rows are packed oldest-to-newest; invalid tail columns are -1.
    HCA/CSA use the original sliding KV cache, so logical positions are first
    mapped to the window ring slot before lowering through the block table.
    """
    if positions.ndim != 2:
        ...
        for s in range(seq):
            ...
            for pos in range(start, abs_pos + 1):
                if pos in overlay_positions:
                    continue
                ring_slot = pos % window
                logical_blk = ring_slot // block_size
                intra = ring_slot % block_size
                if logical_blk >= table_i64.shape[1]:
                    continue
                blk = int(table_i64[b, logical_blk].item())
                if blk >= 0:
