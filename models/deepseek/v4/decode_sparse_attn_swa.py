...
def sparse_attn_swa(
    ...
    cache_insert_tids: pl.Array[CACHE_INSERT_BLOCKS, pl.TASK_ID],
    ...
    with pl.spmd(T, name_hint="swa_gather_kv", deps=[cache_insert_tids[0]]) as gather_tid:
...
def sparse_attn_test(
    ...
    fixture_ready = pl.create_tensor([1, 8], dtype=pl.FP32)
    cache_insert_tids = pl.array.create(CACHE_INSERT_BLOCKS, pl.TASK_ID)
    with pl.at(level=pl.Level.CORE_GROUP, name_hint="swa_fixture_ready") as ready_tid:
        fixture_ready[0:1, 0:8] = pl.full([1, 8], dtype=pl.FP32, value=0.0)
    cache_insert_tids[0] = ready_tid
